...
//			$db_query	= "SELECT E.* FROM (SELECT A.id, A.tracking_number, A.docket, B.id tracking_number_id, B.status_id, B.date_stamp, B.remark, B.location, B.image_path, C.name status_name, C.precedence, D.name sender_station, H.name receiver_station FROM tracking_numbers A LEFT JOIN tracking_status B ON (A.id = B.tracking_number_id) LEFT JOIN status C ON (B.status_id = C.id) LEFT JOIN stations D ON (A.sender_station = D.id) LEFT JOIN stations H ON (A.receiver_station = H.id)) E WHERE E.tracking_number_id IN (SELECT MAX(A.id) FROM tracking_status A INNER JOIN (SELECT tracking_number_id, MAX(date_stamp) date_stamp FROM tracking_status GROUP BY tracking_number_id) B ON (A.tracking_number_id = B.tracking_number_id AND A.date_stamp = B.date_stamp) GROUP BY A.tracking_number_id) ORDER BY E.date_stamp DESC, E.precedence ASC LIMIT 1500";
			$db_query = "SELECT E.* FROM (
							SELECT A.id, A.tracking_number, A.docket, B.id tracking_number_id, B.status_id, B.date_stamp, B.remark, B.location, B.image_path, C.name status_name, C.precedence, D.name sender_station, H.name receiver_station 
							FROM tracking_numbers AS A 
							LEFT JOIN tracking_status AS B ON (A.id = B.tracking_number_id) 
							LEFT JOIN status C ON (B.status_id = C.id) 
							LEFT JOIN stations D ON (A.sender_station = D.id) 
							LEFT JOIN stations H ON (A.receiver_station = H.id)) AS E 
							WHERE E.tracking_number_id IN (SELECT id FROM
							(SELECT tracking_number_id, MAX(id) AS id FROM tracking_status  WHERE tracking_number_id != '' GROUP BY tracking_number_id) AS B)
							ORDER BY E.id DESC, E.date_stamp DESC, E.precedence ASC LIMIT 200;";
		}else{
			...
			$db_query   = "SELECT E.* FROM (SELECT A.id, A.tracking_number, A.docket, B.id tracking_number_id, B.status_id, B.date_stamp, B.remark, B.location, B.image_path, C.name status_name, C.precedence, D.name sender_station, H.name receiver_station 
							FROM tracking_numbers A 
							LEFT JOIN tracking_status B ON (A.id = B.tracking_number_id) 
							LEFT JOIN status C ON (B.status_id = C.id) 
							LEFT JOIN stations D ON (A.sender_station = D.id) 
							LEFT JOIN stations H ON (A.receiver_station = H.id) 
							WHERE (A.sender_station = ".$userid." OR A.receiver_station = ".$userid.") OR (A.created_by = ".$userid." AND A.creator_type = '".$level."')) E 
							WHERE E.tracking_number_id IN 
							(SELECT id FROM
							(SELECT tracking_number_id, MAX(id) AS id FROM tracking_status  WHERE tracking_number_id != '' GROUP BY tracking_number_id) AS B)
							ORDER BY E.id DESC, E.date_stamp DESC, E.precedence ASC  LIMIT 200";
		}
...
//	function fetch_data($userid, $level, $search)
	function get_search_data($userid, $level, $search)
	 {
	 	
	 	/*AND (E.tracking_number LIKE '%".$search."%' OR E.docket  LIKE '%".$search."%' OR E.tracking_number_id LIKE '%".$search."%' OR E.status_id LIKE '%".$search."%' OR E.date_stamp  LIKE '%".$search."%' OR  E.remark LIKE '%".$search."%' OR E.location LIKE '%".$search."%' OR E.image_path LIKE '%".$search."%' OR 
 							E.status_name  LIKE '%".$search."%' OR E.precedence LIKE '%".$search."%' OR E.sender_station LIKE '%".$search."%' OR E.receiver_station  LIKE '%".$search."%')
							*/
	 	if($level == '0'){
			/*$db_query	= "SELECT E.* FROM (SELECT A.id, A.tracking_number, A.docket, B.id tracking_number_id, B.status_id, B.date_stamp, B.remark, B.location, B.image_path, C.name status_name, C.precedence, D.name sender_station, H.name receiver_station FROM tracking_numbers A 
							LEFT JOIN tracking_status B ON (A.id = B.tracking_number_id) 
							LEFT JOIN status C ON (B.status_id = C.id) 
							LEFT JOIN stations D ON (A.sender_station = D.id) 
							LEFT JOIN stations H ON (A.receiver_station = H.id)) E 
							WHERE E.tracking_number_id IN (SELECT MAX(A.id) FROM tracking_status A 
							INNER JOIN (SELECT tracking_number_id, MAX(date_stamp) date_stamp FROM tracking_status GROUP BY tracking_number_id) B 
							ON (A.tracking_number_id = B.tracking_number_id AND A.date_stamp = B.date_stamp) GROUP BY A.tracking_number_id) 
							AND (E.tracking_number LIKE '%".$search."%' OR E.sender_station LIKE '%".$search."%' OR E.receiver_station  LIKE '%".$search."%')
							ORDER BY E.date_stamp DESC, E.precedence ASC";*/
			$db_query	= "SELECT E.* FROM (SELECT A.id, A.tracking_number, A.docket, B.id tracking_number_id, B.status_id, B.date_stamp, B.remark, B.location, B.image_path, C.name status_name, C.precedence, D.name sender_station, H.name receiver_station 
							FROM tracking_numbers A 
							LEFT JOIN tracking_status B ON (A.id = B.tracking_number_id) 
							LEFT JOIN status C ON (B.status_id = C.id) 
							...
							(SELECT tracking_number_id, MAX(id) AS id FROM tracking_status  WHERE tracking_number_id != '' GROUP BY tracking_number_id) AS B) 
							AND (E.tracking_number LIKE '%".$search."%' OR E.sender_station LIKE '%".$search."%' OR E.receiver_station  LIKE '%".$search."%')
							ORDER BY  E.id DESC, E.date_stamp DESC, E.precedence ASC;";
		}else{
			/*$db_query	= "SELECT E.* FROM (SELECT A.id, A.tracking_number, A.docket, B.id tracking_number_id, B.status_id, B.date_stamp, B.remark, B.location, B.image_path, C.name status_name, C.precedence, D.name sender_station, H.name receiver_station FROM tracking_numbers A 
							LEFT JOIN tracking_status B ON (A.id = B.tracking_number_id) 
							LEFT JOIN status C ON (B.status_id = C.id) 
							LEFT JOIN stations D ON (A.sender_station = D.id) 
							LEFT JOIN stations H ON (A.receiver_station = H.id) 
							WHERE (A.sender_station = ".$userid." OR A.receiver_station = ".$userid.") OR (A.created_by = ".$userid." and A.creator_type = '".$level."')) E WHERE E.tracking_number_id IN (SELECT MAX(A.id) FROM tracking_status A 
							INNER JOIN (SELECT tracking_number_id, MAX(date_stamp) date_stamp FROM tracking_status GROUP BY tracking_number_id) B 
							ON (A.tracking_number_id = B.tracking_number_id AND A.date_stamp = B.date_stamp) 
							GROUP BY A.tracking_number_id) 
							AND (E.tracking_number LIKE '%".$search."%' OR E.sender_station LIKE '%".$search."%' OR E.receiver_station  LIKE '%".$search."%')
							ORDER BY E.date_stamp DESC, E.precedence ASC";*/
			$db_query	= "SELECT E.* FROM (SELECT A.id, A.tracking_number, A.docket, B.id tracking_number_id, B.status_id, B.date_stamp, B.remark, B.location, B.image_path, C.name status_name, C.precedence, D.name sender_station, H.name receiver_station FROM tracking_numbers A 
							LEFT JOIN tracking_status B ON (A.id = B.tracking_number_id) 
...
//		$db_query	= "select * from status";
		$db_query	= "SELECT * FROM status ORDER BY precedence";
...
*/
	...
	public function get_all_trackingnumbers_bycustomer($id) {
		...
		$db_query	= "SELECT E.* FROM (SELECT A.id, A.tracking_number, A.docket, B.status_id, B.date_stamp, B.remark, B.location, B.image_path, C.name status_name, C.precedence, D.name sender_station, H.name receiver_station FROM (SELECT * FROM tracking_numbers WHERE sender_id = ".$id." OR receiver_id = ".$id.") A LEFT JOIN tracking_status B ON (A.id = B.tracking_number_id) LEFT JOIN status C ON (B.status_id = C.id) LEFT JOIN stations D ON (A.sender_station = D.id) LEFT JOIN stations H ON (A.receiver_station = H.id)) E INNER JOIN (SELECT A.tracking_number_id, MAX(B.precedence) precedence FROM tracking_status A LEFT JOIN status B ON (A.status_id = B.id) GROUP BY A.tracking_number_id) F ON (E.id = F.tracking_number_id AND E.precedence = F.precedence) ORDER BY E.date_stamp DESC";
...
//		date_default_timezone_set('UTC');
		...
		foreach($result as $row){
			...
			$data[] = array(
				...
				'date_stamp' => date('d M Y', $row->date_stamp),
//				'date_stamp' => date('d M Y', 1610035200),
	...
	public function get_trackingnumbers_pickup($userid, $level) {
