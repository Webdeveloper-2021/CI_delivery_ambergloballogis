...
Class Customers_model extends CI_Model {
	...
	public function get_search_data($userid, $level, $search) {
		...
		if($level == '0'){
			...
			$db_query	.=" WHERE username LIKE '%$search%' OR name LIKE '%$search%' OR email LIKE '%$search%' OR contact_number LIKE '%$search%' OR address LIKE '%$search%' limit 1000 ";
		}else{
			$db_query	= "select * from customers where created_by = ".$userid." and creator_type = '".$level."'";
			...
			$db_query	.=" AND (username LIKE '%$search%' OR name LIKE '%$search%' OR email LIKE '%$search%' OR contact_number LIKE '%$search%' OR address LIKE '%$search%') limit 1000";
		}
		$query	= $this->db->query($db_query);
		return $query->result();
	}
	...
	public function get_customers_count() {
		...
		$db_query	= "select MAX(id) as maxid from customers;";
		$query	= $this->db->query($db_query);
		$ret  =$query->row();
		return $ret->maxid;
	}	
