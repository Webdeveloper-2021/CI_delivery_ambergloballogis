...
class Trackingnumbers extends CI_Controller {
	...
	public function loadData($record=0) {
		...
		if($record != 0){
      	...
      	$recordCount = $this->trackingnumbers_model->getRecordCount( $this->userid, $this->level);
      	$empRecord = $this->trackingnumbers_model->getRecord($record,$recordPerPage, $this->userid, $this->level);
      	$config['base_url'] = base_url().'index.php/admin/Trackingnumbers/loadData';
